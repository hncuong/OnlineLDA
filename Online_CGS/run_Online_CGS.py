...
class runOnlineCGS:
    ...
    def run(self):
        ...
        online_cgs = Online_CGS.OnlineCGS(self.settings['num_docs'], self.settings['num_terms'], 
                                          self.settings['num_topics'], self.settings['alpha'],
                                          self.settings['eta'], self.settings['tau0'], 
                                          self.settings['kappa'], self.settings['burn_in'], 
        ...
        while i < self.settings['iter_train']:
            ...
            while True:
                j += 1
                (wordtks, lengths) = utilities.read_minibatch_list_frequencies(datafp, self.settings['batch_size'])
                # Stop condition
                if len(wordtks) == 0:
                    break
                # 
                print'---num_minibatch:%d---'%(j)
                (time_e, time_m, theta) = online_cgs.static_online(self.settings['batch_size'], wordtks, lengths)
                # Compute sparsity
                sparsity = utilities.compute_sparsity(theta, theta.shape[0], theta.shape[1], 't')
                # Compute perplexities
                LD2 = utilities.compute_perplexities_vb(online_cgs._lambda, self.settings['alpha'], self.settings['eta'], 
                ...
                # Write files
                utilities.write_file(i, j, online_cgs._lambda, time_e, time_m, theta, sparsity, LD2, list_tops, self.tops, 
                                     self.model_folder)
            datafp.close()
        # Write settings
        print'write setting ...'
